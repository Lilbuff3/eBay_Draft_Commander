...
- **Selling costs — one fee formula** — `core/selling_costs.py` owns `fees(amount)` (FVF + $0.30), `net(amount, ship, cogs)` and `ship_cost()` (live-read `SOURCING_SHIP_COST`). It used to be written six ways — `jobs_api` profit breakdown, `ledger.estimate_net`, `sourcing.compute_verdict`, `pricing_engine` twice, and `frontend/src/lib/fees.ts` — and the copies **had already drifted**: `fees.ts` claimed to mirror `SOURCING_SHIP_COST` ($5.00) while carrying $6.50, so the capture-sheet scoreboard disagreed with the Profit tab. Three ship numbers exist and are **not** interchangeable: `ai_data['shipping_cost']` (per-item weight/category tier, what the item drawer uses), `SOURCING_SHIP_COST` (flat actual-cost estimate, what the ledger and sourcing verdict use), `ESTIMATED_SHIPPING_COST` (buffer added to the *list price* under free shipping — a price input, never a cost). `fees()` returns 0 for a non-positive amount, so a $0 order no longer reports the $0.30 processing fee. `fees.ts` still hand-mirrors the numbers (marked `ponytail:`); `tests/unit/test_selling_costs.py` reads that file and fails if it drifts again.
