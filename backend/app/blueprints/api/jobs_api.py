...
def get_job_details(job_id):
    ...
    # COGS is optional. net_profit/net_margin_pct stay None when it is unknown
    # rather than reporting an unpaid-for item as pure profit. Routed through
    # selling_costs.net like every other fee figure -- no local formula.
    try:
        cogs_val = _parse_cogs((job.job_metadata or {}).get('cogs'))
    except ValueError:
        cogs_val = None
    net_profit = (round(selling_costs.net(listing_price, shipping_cost_val, cogs_val), 2)
                  if cogs_val is not None and listing_price > 0 else None)
    net_margin_pct = (round((net_profit / listing_price) * 100, 1)
                      if net_profit is not None and listing_price > 0 else None)
...
def upload_files():
    ...
    try:
        ...
        # A capture note can carry its own "paid 5" token. _extract_cogs strips
        # it from the note so the price never biases the Gemini vision prompt.
        # An explicit cogs form field wins; the note is only a fallback.
