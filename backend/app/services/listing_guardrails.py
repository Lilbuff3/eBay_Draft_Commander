...
def apply_pre_listing_guardrails(
    ...
    # Separate try from the aspect pass above: title hygiene must still run when
    # normalize_aspects raises, or a crash there ships the raw AI title unclean.
    try:
        job.title = optimize_cassini_title(
