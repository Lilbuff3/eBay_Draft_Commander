...
export function MobileReviewDeck({
    ...
    useEffect(() => {
        if (currentItem) {
            const ai = (currentItem.ai_data || {}) as Record<string, unknown>
            const aiCondition = ai.condition as { state?: string } | string | undefined
            setTitle(currentItem.display_name || currentItem.name || '')
            setPrice(String(currentItem.price || ai.suggested_price || ai.price || '0.00'))
            setCondition(
                currentItem.condition
                || (typeof aiCondition === 'object' ? aiCondition?.state : aiCondition)
                || 'Used - Good'
            )
            const rawCogs = currentItem.cogs ?? ai.cogs
            setCogs(rawCogs !== undefined && rawCogs !== null ? String(rawCogs) : '')
            setShippingCost(ai.shipping_cost ? parseFloat(String(ai.shipping_cost)) : 6.50)
        }
    }, [currentItem])
                ...
                >
                    ...
                    <div className="p-4 sm:p-5 space-y-4">
                        ...
                        {(() => {
                            ...
                            const aiData = (currentItem.ai_data || {}) as Record<string, unknown>
                            const seoMetrics = (aiData.seo_metrics as { seo_score?: number; total_aspects_count?: number } | undefined) || {
                            ...
                            const validEntries = Object.entries(specifics).filter(([, v]) => v && v !== 'Does Not Apply' && String(v).trim())
