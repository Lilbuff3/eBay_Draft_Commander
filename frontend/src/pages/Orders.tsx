    ...
    return (
        ...
        >
            ...
            <div className="px-4 sm:px-6 py-4 border-b border-stone-100 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[11px] text-stone-400 mt-0.5">Sold on eBay · last 90 days</p>
                    </div>
                </div>
                <Button variant="ghost" size="icon" onClick={() => refetch()} disabled={isFetching} aria-label="Refresh orders" title="Refresh">
            ...
            <div className="px-4 sm:px-6 pt-3 pb-2 flex gap-1.5 flex-wrap shrink-0">
                {chips.map(c => (
                    <button
                        ...
                        className={`px-2.5 py-1 rounded-lg text-[12px] font-semibold transition ${filter === c.key ? 'bg-ink-800 text-paper' : 'bg-white border border-stone-200 text-stone-500 hover:border-stone-300'}`}
                    >
                        {c.label} {c.n}
                    </button>
                ))}
            ...
            <div className="px-4 sm:px-6 pb-3 shrink-0">
                <Input placeholder="Search by item, buyer, or order ID…" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="bg-white" />
            </div>
