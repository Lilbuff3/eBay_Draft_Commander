...
def test_condition_validation_dict_input():
    """Regression test: AI returns condition as dict, not string (caused [object Object] bug)."""
    # AI response format: {"state": "Used - Like New", "wear_details": "...", "accessories_found": [...]}
    assert validate_condition({"state": "Used - Like New", "wear_details": "pristine"}) == "USED_EXCELLENT"
    assert validate_condition({"state": "New", "wear_details": "sealed"}) == "NEW"
    assert validate_condition({"state": "Used - Good", "accessories_found": []}) == "USED_GOOD"
    assert validate_condition({"state": "For Parts"}) == "FOR_PARTS_OR_NOT_WORKING"

    # Fallback keys
    assert validate_condition({"value": "USED_VERY_GOOD"}) == "USED_VERY_GOOD"

    # Empty dict falls back to default
    assert validate_condition({}) == "USED_GOOD"
    assert validate_condition({"wear_details": "some text"}) == "USED_GOOD"

def test_condition_validation_used_like_new():
    """Regression: 'Used - Like New' normalizes to USED_LIKE_NEW which must map to USED_EXCELLENT."""
    assert validate_condition("Used - Like New") == "USED_EXCELLENT"
    assert validate_condition("USED_LIKE_NEW") == "USED_EXCELLENT"
    assert validate_condition("like new") == "USED_EXCELLENT"
